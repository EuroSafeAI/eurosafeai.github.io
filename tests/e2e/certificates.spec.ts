/**
 * Integration tests for /certificate/[slug] download pages.
 *
...
const CERT_DIR = resolve(process.cwd(), 'public/certificate')
...
//       const res = await page.goto(`/certificate/${ANCHOR_SLUG}`)
//       expect(res?.status()).toBe(200)
//     })

//     test('displays the model name as a heading', async ({ page }) => {
//       await page.goto(`/certificate/${ANCHOR_SLUG}`)
...
//       await expect(btn).toHaveAttribute('href', `/certificate/${ANCHOR_SLUG}.pdf`)
...
//     test('"back to leaderboard" link navigates to /certificate', async ({ page }) => {
//       await page.goto(`/certificate/${ANCHOR_SLUG}`)
...
//       await page.goto('/certificate/this-model-does-not-exist-xyz-999', {
...
//       const res = await page.goto('/certificate/this-model-does-not-exist-xyz-999')
...
//   test.describe('all generated certificates', () => {
    ...
    // if (slugs.length === 0) {
    //   test('skipped — no certificates found in public/certificate/', async () => {
    ...
    //     const res = await page.goto(`/certificate/${slug}`)
    //     expect(res?.status()).toBe(200)

    //     const btn = page.getByRole('link', { name: /download certificate/i })
    //     await expect(btn).toBeVisible()
    //     await expect(btn).toHaveAttribute('href', `/certificate/${slug}.pdf`)
    //     await expect(btn).toHaveAttribute('download')
    //   })
    // }
//   })
